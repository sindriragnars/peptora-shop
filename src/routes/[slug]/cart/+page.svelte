<script lang="ts">
	...
	// Resolve each cart line to its current product. Filtered for products
	// that disappeared (deleted from catalog between add and view).
	const lines = $derived(
		cart.items
			.map((item) => ({
				item,
				product: getProduct(tenant.slug, item.productId)
			}))
